...
- 资源：`Resources/sample.wav`（与 Android 相同的 2s 440Hz WAV）。
- 示例代码：`Sources/SpectrumHost.swift` 展示如何用 `AVAudioEngine` 播放 bundle WAV，并调用 `SoundwaveCore.SpectrumEngine` 计算频谱。
...
4. 拖入 `Sources/SpectrumHost.swift`，在 `AppDelegate`/`SceneDelegate` 或 ViewModel 中实例化 `SpectrumHost` 并调用 `start()`。
5. 运行后控制台会输出频谱 bins 数和前三个幅度示例；如需替换音频，可改换 bundle 资源。
