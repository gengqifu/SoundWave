...
/// Android 端 FFT 契约：频谱事件能反映 1kHz 正弦主峰，binHz 与采样率/窗口一致。
void main() {
  ...
  test('native FFT emits peak near 1kHz', () async {
    // 生成 1kHz 正弦，模拟 native FFT（用 Dart 简单 DFT）得到 bins。
    const sampleRate = 48000;
    const windowSize = 1024;
    final samples = List<double>.generate(
        windowSize,
        (n) =>
            math.sin(2 * math.pi * 1000 * n / sampleRate)); // 1kHz, mono, float64
    ...
    final result = controller.spectrumBuffer.drain(5);
    expect(result.frames, isNotEmpty);
    final frame = result.frames.first;
    expect(frame.binHz, closeTo(binHz, 1e-6));

    // 峰值频率应在 1kHz 附近（容忍一个 bin）。
    final peakIndex = _peakBin(frame.bins);
    final peakHz = peakIndex * frame.binHz;
