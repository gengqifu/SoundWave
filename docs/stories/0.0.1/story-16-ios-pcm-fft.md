...
- ✔ [6] 资源加载与配置：示例增加本地 asset `sample.mp3` 拷贝到沙盒并填充路径的按钮；iOS Info.plist 增加 ATS 例外以便开发阶段 HTTP 访问，保持播放/旁路兼容。
- ✔ [7] 状态/生命周期：处理中断/路由/后台前台切换时，暂停/恢复旁路，避免旧数据污染；释放时移除 tap 与观察者，并添加日志辅助诊断。
