...
- ✅ [4] iOS 保持 vDSP 默认，增加可选 Swift FFT（替代路径），窗口/归一化（2/(N*E_window)）与 Android 对齐，可配置 overlap（后续如需）。
- ✖️ [5] 跨端一致性测试与阈值校验：统一窗口/nfft/overlap/归一化，用同一组测试信号（单频/双频/白噪/扫频）做误差评估（L2/最大值）。

## 完成标准（DoD）
- ✖️ [6] 跨端 FFT 对齐测试通过，谱差异在容差内，有测试报告或黄金比对脚本。
- ✅ [7] 相关单测/集成测/构建门禁通过。（flutter analyze/test 通过；JNI/KissFFT 构建随 Android 构建完成）
- ✅ [8] 文档更新 FFT 选型、归一化公式与开关说明。***


## 跨端对齐报告
- 详情：docs/fft_cross_platform_report.md
