...
- ✖️ [6] Android 频域处理：在 core 集成 KissFFT，完成窗口化/FFT 计算，adapter 仅做 PCM 接入，桥接 spectrum 事件到 Flutter。
- ✖️ [7] iOS 频域处理：在 core 集成 KissFFT（与 Android 统一），完成窗口化/FFT 计算，tap 层仅推送 PCM，桥接 spectrum 事件到 Flutter。
...
## 完成标准（DoD）
- ✖️ [11] 构建与宿主集成验证通过，核心 API/事件可用。
- ✖️ [12] 发布脚本与文档齐全，版本号/依赖声明清晰。 
