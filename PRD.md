...
- MVP
  ...
  - 跨平台音频核心：Android/iOS 共用同一套 C/C++ 算法与音频处理库，通过平台桥接层暴露统一接口。
- 扩展（非 MVP，规划项）
  - 立体声通道分离显示、能量峰值指示。
  - 录音输入可视化（环回/麦克风）。
  - 波形截图/导出。
...
- 音频核心：C/C++（FFmpeg 或平台解码器），环形缓冲、窗口化 + FFT（KissFFT/vDSP/Android FFT）；核心算法库需跨平台通用，Android/iOS 共用代码基线，仅在平台层做少量 IO/权限适配。
- 构建：CMake 管理原生核心；Flutter 构建移动端包。

## 9. 依赖与外部接口
- 第三方库：FFmpeg（可选平台解码器替代）、KissFFT/平台 FFT。
