...
## 测试优先（TDD）
- ✖ [1] 先编写 CI 流程的验证用例：人为降低覆盖率触发失败，格式化失败触发失败（本地脚本模拟）。
- ✖ [2] 本地脚本模拟 CI 关键步骤，确保可复现（Flutter analyze/test、native gtest）。
...
- ✖ [4] 覆盖率收集：Dart 覆盖率（lcov），C++ 覆盖率（lcov/llvm-cov 占位），可选合并报告。
- ✖ [5] 阈值：设定最低覆盖率（先占位，可后续收紧），未达则失败；构建缓存优化。
